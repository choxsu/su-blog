...
public class MetaBuilder {
     ...
     * @throws SQLException
     */
    protected void buildColumnRemarks(TableMeta tableMeta) throws SQLException {

        ResultSet rst = dbMeta.getColumns(conn.getCatalog(), null, tableMeta.name, null);
        List<ColumnMeta> columnMetas = tableMeta.columnMetas;
        while (rst.next()) {
            for (ColumnMeta columnMeta : columnMetas) {
                if (Objects.equals(rst.getString("COLUMN_NAME"), columnMeta.name)) {
                    columnMeta.remarks = rst.getString("REMARKS");
